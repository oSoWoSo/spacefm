...
VFSAppDesktop::VFSAppDesktop(const std::string& open_file_name)
{
    ...
    else
    {
        ...
        std::string relative_path = vfs_build_path("applications", m_file_name);
        char* full_path_c;
...
VFSAppDesktop::open_multiple_files()
{
    if (!m_exec.empty())
    {
        ...
        if (ztd::contains_any(m_exec, keys))
            return true;
    }
    return false;
}
...
VFSAppDesktop::translate_app_exec_to_command_line(std::vector<std::string>& file_list)
{
    ...
    if (ztd::contains_any(cmd, open_files_keys))
    {
        std::string tmp;
        ...
        cmd = ztd::replace_multiple(cmd, open_files_keys, tmp);
        add_files = true;
    }

    std::vector<std::string> open_file_keys{"%f", "%u"};
    if (ztd::contains_any(cmd, open_file_keys))
    {
        std::string tmp;
        for (std::string file: file_list)
        {
        ...
        cmd = ztd::replace_multiple(cmd, open_file_keys, tmp);
        add_files = true;
    }
